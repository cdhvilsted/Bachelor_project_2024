...
fname = 'examples/data/02IT_fam-markers.hdf5'
...
ch_names2 = ['Fp1', 'Fp2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7', 'T8', 'P7', 'P8', 'Fz', 'Cz', 'Pz']
info = mne.create_info(ch_names2, 1, ch_types='eeg')
...
for ax, name, topo in zip(axes, names, topos_to_plot):
    vmin = np.nanmin(topo[ch_names])
    vmax = np.nanmax(topo[ch_names])
    #topo[non_scalp] = vmin
    ...
    im, _ = mne.viz.topomap.plot_topomap(
        topo, pos=info, axes=ax,
        cmap=cmap, image_interp='nearest', sensors=True, contours=0, show=False)
